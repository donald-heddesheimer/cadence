// Compiled with -DCADENCE_DISABLE. Everything below must still build, and the
// macros must collect nothing at all -- not "collect and throw away".
...
int main() {
  ...
  // Nothing was recorded, so nothing can be reported. Note that Configure()
  // never ran either: CADENCE_CONFIGURE compiled out with the rest.
