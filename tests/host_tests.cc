// Host-only tests for cadence. No CUDA, no GPU, no driver required: these
// cover the logic that decides whether the reported numbers are correct.
//
// Deliberately dependency-free -- a test framework would be the heaviest thing
// in a header-only repo.
...
// Finds one row of a Snapshot() by label and scope.
// Takes the snapshot itself rather than a reference into one. Returning a
// pointer into a vector the caller passed as a temporary is a use-after-free
// waiting for the first person who writes Find(cadence::Snapshot(), ...).
...
void TestWarmupDiscard() {
  ...
  // The first three observations are the expensive ones (context creation, JIT,
  // library autotuning); they must not reach the statistics.
...
void TestWarmupPersistsAcrossFlushes() {
  ...
  cadence::detail::Registry& registry = cadence::detail::Registry::Instance();
  // One observation per flush: the counter has to survive the flush boundary,
  // which is the whole point of keeping it in the registry.
  for (int i = 0; i < 4; ++i) {
...
void TestCsvOutput() {
  ...
  // Provenance header: warmup and clock state must be visible in the artifact,
  // so nobody quotes a throttled run as gospel.
...
// Interning has to key on the characters, not on the pointer: the same literal
// in two translation units is two addresses, and a label built at runtime is a
// third. All of them are one label.
...
// A label interned once must survive the table growing under it. The handle a
// call site caches points into the table, so a container that moved its
// elements would leave every earlier call site holding a dangling name.
