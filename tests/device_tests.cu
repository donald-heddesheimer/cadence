...
//
// The host tests cover arithmetic. These cover the things that can only be
// wrong once a driver is involved: whether a measured duration corresponds to
// the work it brackets, whether stage chaining hands each event back to the
// pool exactly once, and whether the contract chaining trades away is the one
// documented.
//
