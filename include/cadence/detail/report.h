...
inline void WriteReportHeader(std::ostream& out, const Config& config, const RunInfo& info,
                              std::size_t failedRecords) {
  out << "# cadence report\n";
  ...
  out << "\n";
  out << "# max_sm_clock_mhz: " << info.maxSmClockMhz
      << "  max_mem_clock_mhz: " << info.maxMemClockMhz << "\n";
...
inline void WriteStatsCsv(std::ostream& out, const std::vector<Stats>& stats) {
  out << "label,scope,count,warmup_discarded,mean_ms,min_ms,p50_ms,p95_ms,max_ms,"
         "stddev_ms,jitter_ms\n";
