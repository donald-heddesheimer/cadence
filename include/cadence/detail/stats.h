...
// Which side of the launch a measurement came from.
...
inline Stats ComputeStats(const std::string& label, ScopeKind kind, std::vector<double> samples,
  ...
  stats.stddevMs = samples.size() > 1
                       ? std::sqrt(sumSquaredDeviation / static_cast<double>(samples.size() - 1))
                       : 0.0;
