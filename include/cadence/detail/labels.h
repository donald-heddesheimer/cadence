...
//
// A label is resolved to a small integer once per call site, not once per
// execution: the scope macros hold the result in a function-local static, so
// the second and every later pass through a scope costs one guard-variable
// load. Three things follow from that.
//
...
//
// Constructing a scope class directly, rather than through the macros, interns
// on every construction and takes a lock to do it. That is the slow path and
// the reason the macros exist.
#pragma once
...
// What a call site caches. The counter and the name are resolved alongside the id so that neither sampling nor the NVTX passthrough has to index a growing container from the hot path.
struct LabelHandle {
...
class LabelTable {
 public:
  static LabelTable& Instance() {
    // Leaked on purpose: labels are referenced by the exit-time report and by thread-local state whose destruction order against a function-local static is not something worth betting a crash on.
  ...
  LabelHandle Intern(const char* label) {
    ...
    // Deques, not vectors: a handle handed out earlier points into these, and a deque leaves existing elements exactly where they are when it grows. A vector<string> would move its elements, and a short string keeps its characters inside the string object itself -- so c_str() would dangle on the first reallocation.
